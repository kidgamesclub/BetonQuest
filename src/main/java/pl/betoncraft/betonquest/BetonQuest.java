...
public final class BetonQuest extends JavaPlugin {
	...
	private Map<String, Class<? extends Condition>> conditionTypes = new HashMap<>();
	private Map<String, Class<? extends QuestEvent>> eventTypes = new HashMap<>();
	private Map<String, Class<? extends Objective>> objectiveTypes = new HashMap<>();
	private Map<String, Class<? extends ConversationIO>> convIOTypes = new HashMap<>();
	private Map<String, Class<? extends Variable>> variableTypes = new HashMap<>();

	private Map<ConditionID, Condition> conditions = new HashMap<>();
	private Map<EventID, QuestEvent> events = new HashMap<>();
			...
			public void run() {
				...
				for (Player player : Bukkit.getOnlinePlayers()) {
					...
					if (playerData.getConversation() != null)
						new ConversationResumer(instance, playerID, playerData.getConversation());
				}
			}
		});
  ...
  public static boolean condition(String playerID, ConditionID conditionID) {
		...
		for (Entry<ConditionID, Condition> e : getInstance().conditions.entrySet()) {
	...
	public static void event(String playerID, EventID eventID) {
		...
		for (Entry<EventID, QuestEvent> e : getInstance().events.entrySet()) {
	...
	public static void newObjective(String playerID, ObjectiveID objectiveID) {
		...
		for (Entry<ObjectiveID, Objective> e : getInstance().objectives.entrySet()) {
	...
	public static Variable createVariable(ConfigPackage pack, String instruction) throws InstructionParseException {
		...
		for (Entry<VariableID, Variable> e : getInstance().variables.entrySet()) {
		...
		Class<? extends Variable> variableClass = getInstance().variableTypes.get(parts[0]);
		// if it's null then there is no such type registered, log an error
		...
		try {
			...
			getInstance().variables.put(ID, variable);
