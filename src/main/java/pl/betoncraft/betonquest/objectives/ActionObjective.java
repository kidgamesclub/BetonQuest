...
/**
 * Player has to click on block (or air). Left click, right click and any one of them is supported.
 *
 * @author Jakub Sapalski
 */
public class ActionObjective extends Objective<Location> implements Listener {
  ...
  public String getReadableProperty(String name, String playerID) {
    return getProperty(name, playerID)
          .map(location ->
  ...
  @Override
  public Optional<Location> getProperty(String name, String playerID) {
    if ("location".equalsIgnoreCase(name)) {
      return Optional.of(loc)
            .flatMap(loc -> {
              try {
                return Optional.of(loc.getLocation(playerID));
              } catch (QuestRuntimeException e) {
